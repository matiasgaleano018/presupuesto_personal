<?php
    ...
    $ingresos = $selec->seleccionar("SELECT SUM(monto), DATE_FORMAT(fecha, '%c') FROM `movimientos` WHERE `usuario_id` = $usuario_id AND `tipo` = 'ingreso' AND DATE_FORMAT(fecha, 'Y') = DATE_FORMAT(CURDATE(), 'Y') GROUP BY DATE_FORMAT(fecha, '%m')");
    
    $meses = ["Enero", "Febrero", "Marzo", "Abril", "Mayo", "Junio", "Julio", "Agosto", "Septiembre", "Octubre", "Noviembre", "Diciembre"];
    foreach($ingresos as $ingre){
        $mes = $meses[$ingre[1]-1];
        $ingre_json[$mes] = $ingre[0];
    }
