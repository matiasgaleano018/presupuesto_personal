<?php
    ...
    $egresos = $selec->seleccionar("SELECT SUM(monto), categoria_id FROM `movimientos` WHERE `usuario_id` = $usuario_id AND `tipo` = 'egreso' AND DATE_FORMAT(fecha, 'Y/m') = DATE_FORMAT(CURDATE(), 'Y/m') GROUP BY categoria_id");
    
    foreach($egresos as $egre){
        $categoria = $selec->seleccionar("SELECT nombre, color FROM categorias WHERE id = $egre[1]");
        $cate  = !empty($categoria[0][0]) ? $categoria[0][0] : "Otro";
        $color = !empty($categoria[0][1]) ? $categoria[0][1] : "#C7C6C6";
        $valores = array($egre[0], $color);
        $egre_json[$cate] = new NodoArbolDHTML(number_format($egre[0], $decimales_mone, ',', ''), $color);
    }
